...
### 用nvm管理安装node、npm、pnpm

获取最新安装脚本（去 github 查看最新版本）：
```sh
...
nerfonts是带有各种图标的更全的字体。

```sh
mkdir -p ~/.local/share/fonts
```

去[官网](https://www.nerdfonts.com/font-downloads)挑选一款下载，解压缩后放入`~/.local/share/fonts`内。比较推荐的一款是`Iosevka Nerd Font`（带Term的适合给终端用）。
...
[Yazi](https://yazi-rs.github.io)是一个终端文件管理器。去[最新的版本](https://github.com/sxyazi/yazi/releases)下载.deb安装包，建议使用不依赖lib链接的musl版本，名称一般是`yazi-x86_64-unknown-linux-musl.deb
`。

安装一些依赖：
```sh
sudo apt install ffmpeg p7zip-full jq poppler-utils fd-find ripgrep fzf zoxide imagemagick xclip wl-clipboard file
...
:::info[zoxide]
zoxide让你可以用`z`替代`cd`，它具有记忆功能，无需在对应父文件夹中就可以用`z 目标文件夹名称`来跳转。
:::

给你要使用的终端配置你安装好的nerd font，一般是终端右键，首选项里面。最后就是给你的Yazi安装一个好看的皮肤。

### 安装环境管理器

Anaconda比较大，不建议安装。建议Miniconda（官方版本）和Miniforge3（社区版本）二选一。推荐使用后者。
...
EOF
```
使用方法和conda类似，但是把`conda`换成`mamba`。
